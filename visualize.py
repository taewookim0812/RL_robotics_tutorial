...
def plot_rewards(viz, win, reward_step, info=''):
    # print('plot reward!!')

    fig = plt.figure()

    plt.clf()
    rewards_s = torch.tensor(reward_step, dtype=torch.float)
    plt.title('Reward / step ' + info)
    plt.xlabel('Step')
    plt.ylabel('Reward')
    plt.plot(rewards_s.numpy())
    plt.pause(0.001)
    plt.show()
    plt.draw()
...
def load_data(indir, smooth, bin_size, bullet=False):
    datas = []
    if bullet == True:
        infiles = glob.glob(indir + '.monitor.csv')
    else:
...
def visdom_plot(viz, win, folder, game, name, num_steps, bin_size=100, smooth=1, bullet=False):
    if bullet == True:
        tx, ty = load_data(folder + game, smooth, bin_size, bullet)
    else:
        tx, ty = load_data(folder, smooth, bin_size, bullet)
